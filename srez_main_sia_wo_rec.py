...
tf.app.flags.DEFINE_integer('batch_size', 17, "Number of samples per batch.")
tf.app.flags.DEFINE_string('checkpoint_dir', 'checkpoint', "Output folder where checkpoints are dumped.")
tf.app.flags.DEFINE_integer('checkpoint_period', 5000, "Number of batches in between checkpoints")
...
tf.app.flags.DEFINE_float('learning_rate_start', 0.00040, "Starting learning rate used for AdamOptimizer")
...
tf.app.flags.DEFINE_bool('useRecTerm', False,  "Set whether the reconstruction term is used or not")
...
def _train():
    ...
    test_features, test_features2, test_labels, test_labels2, ytlab, fnt1, fnt2, tlen = srez_input_sia.setup_inputs(sess, FLAGS.training_img_dir+"pairwise-val2.txt", image_size=32, crop_size=128, isTest=True, batch_size=17)
